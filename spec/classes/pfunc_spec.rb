...
describe 'pfunc' do
  ...
  shared_examples 'pfunc shared examples' do
    ...
    it {
      is_expected.to contain_notify('pfunc::nope function result')
        .with_message('pfunc::nope function result: |test for doing nothing|')
    }

    it {
      is_expected.to contain_notify('pfunc::split function result')
        .with_message('pfunc::split function result: |[split, me, on, comma]|')
    }

    it {
      is_expected.to contain_notify('pfunc::join function result')
        .with_message('pfunc::join function result: |join,me,with,comma|')
    }

    it {
      is_expected.to contain_notify('pfunc::hex function result')
